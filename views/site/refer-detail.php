...
$this->title = 'รายละเอียดการส่งต่อผู้ป่วย (Refer Out)';
?>
...
<div class="bg-slate-50 min-h-screen p-8 font-sans">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 mb-8">
            <form method="get" action="<?= \yii\helpers\Url::to(['/site/refer-detail']) ?>" class="flex flex-wrap items-end gap-4">
                <input type="hidden" name="r" value="site/refer-detail">
                <input type="hidden" name="type" value="<?= Yii::$app->request->get('type') ?>">
                
                <div class="flex-1 min-w-[200px]">
                    <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-widest">ช่วงวันที่ Refer</label>
                    <div class="relative">
        ...
        <div class="flex justify-between items-center mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-rose-600">10 อันดับโรค Refer Out</h1>
        ...
        <div id="pdf-area" class="space-y-8">
            <div class="bg-white rounded-[2rem] shadow-xl p-8 border border-slate-100">
                <h2 class="text-lg font-bold text-slate-700 mb-6 italic">
                    สถิติการส่งต่อ (<?= date('d/m/Y', strtotime($start_date)) ?> - <?= date('d/m/Y', strtotime($end_date)) ?>)
                </h2>
            ...
            <!-- <div class="bg-white rounded-[2rem] shadow-xl p-8 border border-slate-100">
                <h2 class="text-lg font-bold text-slate-700 mb-4 italic">ตารางรายการโรคที่ถูกส่งต่อมากที่สุด</h2>
                <p class="text-slate-400 font-bold text-sm">จุดบริการ: <span class="text-slate-700"><?= $type_name ?></span></p>
                <nav>---</nav>
                 
                <table id="referTable" class="w-full">
                    <thead>
                        <tr class="text-left border-b border-slate-100 uppercase text-xs tracking-widest text-slate-400">
                            <th class="pb-4">ICD10</th>
                            <th class="pb-4">ชื่อโรคการวินิจฉัย</th>
                            <th class="pb-4 text-right">จำนวน Refer (ครั้ง)</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        <?php foreach ($diagStats as $row): ?>
                        <tr class="hover:bg-slate-50 transition">
                            <td class="py-4 font-bold text-rose-600"><?= $row['icd10'] ?></td>
                            <td class="py-4 text-slate-600 text-sm"><?= $row['diag_name'] ?></td>
                            <td class="py-4 text-right font-black text-slate-800"><?= number_format($row['total']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if(empty($diagStats)): ?>
                        <tr>
                            <td colspan="3" class="py-10 text-center text-slate-400 font-bold">ไม่พบข้อมูลการ Refer ในช่วงวันที่เลือก</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <!-- <h2 class="text-xl font-black text-slate-800 flex items-center gap-2"> -->
                            ...
                            ตารางรายการโรคที่ถูกส่งต่อมากที่สุด
                        </h2>
                        ...
                        <tr>
                            <td colspan="3" class="py-20 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-slate-400 font-bold">ไม่พบข้อมูลการ Refer ในช่วงวันที่เลือก</p>
                                </div>
                            </td>
                        </tr>
                        <?php endif; ?>
         ...
         <div class="bg-white rounded-[2rem] shadow-xl p-8 border border-slate-100 mt-8 overflow-hidden relative">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 relative z-10">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-black text-slate-800 tracking-tight">10 อันดับสถานพยาบาลที่รับส่งต่อ</h2>
            ...
            <div class="lg:col-span-7 bg-slate-50/50 p-6 rounded-[1.5rem] border border-slate-50">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex flex-col items-center justify-center py-12 opacity-40">
                        ...
                        <p class="text-sm font-bold">ไม่มีข้อมูลการส่งต่อ</p>
                    </div>
                        <?php endif; ?>
                </div>
            </div>
        ...
        <div class="bg-white rounded-[2rem] shadow-xl p-8 border border-slate-100 mt-8">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-lg font-bold text-slate-700 italic">Total Refer Department/Point</h2>
                </div>
